<?php 
class manage_area_test extends CI_Controller {

 function __construct() { 
  parent::__construct(); 
  $this->load->library(array('form_validation', 'session', 'email', 'pagination'));
  $this->load->model(array('Managearea_Model_test'));        
...
public function view(){
  ...
  $data['main_content'] = $this->load->view("area/manage_area_view_test",$data,TRUE);
...
//else{
  ...
  if ($radio=='Newcity') {
    ...
    if ($this->form_validation->run() == FALSE ) {
      ...
      $data = array('Name' => $text_city,
        ...
        // print_r($data);die;
      $this->Managearea_Model_test->insert_new($data);
      $this->session->set_flashdata('city_sucess','City inserted sucessfully');
      redirect('manage_area_test/view');
  ...
  ///////////////////// EXISTITNG CITY
    ...
    $this->form_validation->set_rules($form_validate_rules1);
                      ...
                      // print_r($data);die;
                        $this->Managearea_Model_test->insert_exist($data);
...
public function city_check(){
  ...
  $rows = $this->Managearea_Model_test->city_chks($ciy_alone);
  if ($rows != 0)
  {
...
public function suburb_check(){
  ...
  $rows = $this->Managearea_Model_test->suburb_chks($suburb_alone);
  if ($rows != 0)
  {
    $this->session->set_flashdata('radio_flash',$radio);
...
public function fetch_city(){
  ...
  {
    $this->Managearea_Model_test->fetch_city($this->input->post('ajaxprovince_id'));
  }
...
public function update(){
  ...
  $temp_sel['main_content'] = $this->load->view('area/area_update_test',$temp_sel,TRUE);
...
public function getupdate($areasubID){
  ...
  $this->Managearea_Model_test->update($data,$areasubID);
...
public function suburb_check_update(){
  ...
  $suburb_alone = $this->input->post('Suburb_update');
    $rows = $this->Managearea_Model_test->suburb_chks_update($suburb_alone);
  if ($rows != 0)
  {
    $this->session->set_flashdata('radio_flash',$radio);
    $this->session->set_flashdata('suburb_flash',$suburb_alone);
