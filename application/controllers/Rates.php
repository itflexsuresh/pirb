...
class rates extends CI_Controller {
	...
	function __construct() { 
		...
		$this->load->model(array('Rates_model'));        
	...
	public function view()
	{
		...
		$data['main_content'] = $this->load->view('rates/rates_view',$data,TRUE);
	...
	public function update_view()
	{
		...
		$sql = "SELECT `SupplyItem`,`Amount`,`ValidFrom`,`futureammount`,`futuredate` FROM `rates` WHERE `ID`='".$data['rates_id']."'";
		$query = $this->db->query($sql);
		$data['page_title'] = "Rates";
		$data['records'] = $query->result(); 
		$data['main_content'] = $this->load->view('rates/rates_update',$data,TRUE);
		$this->load->view('admin/index', $data);
	}
	public function add_update($role_id){
		...
		if ($this->input->post('radio')=='option1') {
			...
			$this->Rates_model->update($data,$role_id);
			...
			$data = array('SupplyItem' => $rateType, 'futureammount' => $ammountVAT, 'futuredate' => $valid_date );
			//print_r($data);die;
			$this->Rates_model->update($data,$role_id);
			$this->session->unset_userdata('rateID');
			$this->session->set_flashdata('update_sucess','Rates Updated sucessfully'); 
		...
		// $this->Rates_model->update($data,$role_id);
		// $this->session->unset_userdata('rateID');
		// $this->session->set_flashdata('update_sucess','Rates Updated sucessfully',2); 
		// //$this->session->set_flashdata('update_sucess','Rates Updated sucessfully');
		// redirect('rates/view');
