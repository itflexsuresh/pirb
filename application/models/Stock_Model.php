...
class Stock_Model extends CC_Model
{
	...
	public function action($data){
		...
		if(isset($requestdata)){
			...
			$inv_id = $this->db->select('*')->from('coc_orders')->where(['id' => $data['order_id']])->get()->row_array();
			...
			$currency    = $this->config->item('currency');
				...
				// invoice PDF
				 	...
				 	$rowData = $this->Coc_Model->getListPDF('row', ['id' => $inv_id['inv_id'], 'status' => ['0','1']]);

							$rowData1 = $this->Coc_Model->getPermissions('row', ['id' => $inv_id['inv_id'], 'status' => ['0','1']]);
							$rowData2 = $this->Coc_Model->getPermissions1('row', ['id' => $inv_id['inv_id'], 'status' => ['0','1']]);
