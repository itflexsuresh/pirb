			...
			if(isset($data['coc_type'])){
				if($data['coc_type']==1){
					for($i=1;$i<=$data['coc_count'];$i++){
						...
						$this->diaryactivity(['adminid' => $this->getUserID(), 'plumberid' => $data['user_id'], 'actionid' => $this->db->insert_id(), 'action' => '6', 'type' => '1']);
					}		
					...
					for($i=$data['allocate_start'];$i<=$data['allocate_end'];$i++){
						...
						$this->diaryactivity(['adminid' => $this->getUserID(), 'plumberid' => $data['user_id'], 'actionid' => $i, 'action' => '6', 'type' => '1']);
					}
				}
			}			
