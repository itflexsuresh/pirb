...
class Coc_Ordermodel extends CC_Model
{
	public function getCocorderList($type, $requestdata){
		...
		$this->db->join('users_detail t2', 't1.user_id=t2.user_id', 'inner');
		$this->db->join('users_address t3', 't1.user_id=t3.user_id AND t3.type="2"', 'inner');
		$this->db->join('users t4', 't1.user_id=t4.id', 'inner');
		$this->db->join('coc_count cc', 'cc.user_id=t2.user_id','inner');		
